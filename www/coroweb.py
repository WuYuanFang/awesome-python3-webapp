...
def has_request_arg(fn):
	...
	for name, param in params.items():
		if name == 'request':
			found = True
			continue
		if found and (param.kind != inspect.Parameter.VAR_POSITIONAL and param.kind != inspect.Parameter.KEYWORD_ONLY and param.kind != inspect.Paramter.VAR_KEY
			):
...
class RequestHandler(object):
	...
	async def __call__(self, request):
		kw = None
		if self._has_var_kw_arg or self._has_name_kw_args or self._has_request_arg:##change
		...
		if kw is None:
			...
			if not self._has_var_kw_arg and self._has_name_kw_args:##change
				copy = dict()
